...
U_akt = U0;
dU_akt = V0; 
K_akt = K; K_akt(1,1)= K_akt(1,1)+ kb ;
f_si = 0;  
U_lab = U_akt(1);
f_si_lab = f_si;
f_si = 0;  % mivel nincs kezdeti alakvaltozas
k_ti_lab = kb;

%% globalis valtozok, az anyagmodell parameterei
k_pl=0.1*kb;   % aktiv keplekeny merevseg
global mat_param; 
mat_param=[kb, 0.1 ,0.05 ,0.0];
%mat_param(1): kb rugalmas merevseg
%mat_param(2): aktiv keplekeny merevseg 
%              es rugalmas merevseg aranya
%mat_param(3): u_0 (elso folyas)
%mat_param(4): u_p pillanatnyi keplekeny alakvaltozas

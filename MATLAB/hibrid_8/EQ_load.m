function [ upp,tload,input_accel ] = EQ_load(fileToRead)% - foldrenges rekord beolvasasa
%
...
%   interpolalasi tartomanyba:
    if t(i)>tkovetkezo ;
%     ha igen, akkor az interpolalas also es felso hatarat eggyel feljebb leptetjuk
      taktual=taktual+1; 
      if (taktual == Tload_stepnum); break; end; %de csak ha van meg ott teher
      telozo=tload(taktual);tkovetkezo=tload(taktual+1);
    end;  
% - t_ksi-ben relativ idot merunk az interpolalashoz (0 az elejen, 1 a vegen):
